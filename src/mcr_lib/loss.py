...
def R(Z: torch.Tensor, eps_sq: float) -> torch.Tensor:
    ...
    log_det_matrix = log_det_matrix.detach().numpy()
    res = 1/2 * sum(np.log(np.linalg.eigvals(log_det_matrix)))
    return res
...
def R_c(Z: torch.Tensor, Pi: torch.Tensor, eps_sq: float) -> torch.Tensor:
    ...
    log_det_matrix = log_det_matrix.detach().numpy()
    res = sum(trace / (2 * m) * sum(np.log(np.linalg.eigvals(log_det_matrix))))
